...
%   After converting the original image (provided as an argument) into the gradient domain, this
%     function reconstructs the image (using a single pixel as a point of reference) and returns it 
%     to the function's caller
    ...
    for c = 1:channels
        ...
        % Set up the first constraint: equivalence of the horizontal components of the gradients
        for x = 1:width-1
            for y = 1:height
                e = e + 1;
        ...
        % Set up the second constraint: equivalence of the vertical components of the gradients
        for x = 1:width
            for y = 1:height-1
                e = e + 1;
        ...
        % Set up the third constraint: the top left pixel of the reconstructed image should have the
        %   same intensity as the pixel at the same location in the source image
        e = e + 1;
        A(e, pixelIndices(1,1)) = 1;
        b(e) = image(1,1,c);

        % Use the constraints above to solve for the reconstructed image, reshaping it to match the
        %   dimensions of the source image
        reconstructed(:,:,c) = full(reshape(A \ b, [height, width]));
    end
end
