...
%     caller
    ...
    for c = 1:channels
        ...
        % Initalize the constraint matrices (using sparse matrices since
        %   the vast majority of values in the matrices will be zeros)
        A = sparse(height * width * 2 + 1, height * width);
        b = zeros(height * width * 2 + 1, 1);

        % Set up the first constraint: equivalence of the horizontal
        ...
        % Set up the second constraint: equivalence of the vertical
        %   components of the gradients
        for x = 1:width
            for y = 1:height-1
                e = e + 1;
        ...
        % Set up the third constraint: the top left pixel of the
        %   reconstructed image should have the same intensity as the pixel
        %   at the same location in the source image
        e = e + 1;
