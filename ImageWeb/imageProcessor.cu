 ...
 * Last Edit: 5/10/16
 *
 * This file is where most of the work performed by this application is done. Leveraging CUDA with
 *   Thrust and OpenGL, the processImageResource function (defined in imageProcessor.hpp) runs
 *   through a multi-stage pipeline to transform input images and videos into a nonrealistic
 *   triangulation-based rendered version.
 *
 * Broadly speaking, the pipeline has the following stages:
 *   1. Set up CUDA for use and allocate relevant device memory
 *   2. Set up OpenGL, allocating buffers as needed and prepping for interoperation with CUDA
 *   3. For every frame in the input resource image:
 *      a. Copy the frame to the GPU
 *      b. Generate a 2D distribution based on edge data, image intensity, and historical point data
 *      c. Sample points from that distribution
 *      d. Generate a triangulation from those points and assign coloring data to those triangles
 *      e. Call out to OpenGL to draw the triangles
 *      f. Copy the computed frame from the GPU to the CPU; insert it into an output ImageResource
 *   4. Release all system resources held by CUDA, OpenGL, and standard CPU code
 *
 * This file is a candidate for refactoring; work could be more cleanly abstracted and
 *   compartmentalized, and the amount of code in this file (over 700 lines) is somewhat unwieldy
 *   when looking to read the code or make changes.
 */
